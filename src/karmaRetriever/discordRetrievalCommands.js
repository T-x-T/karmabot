...
export default (_client, _commandPrefix) => {
  ...
  client.on("message", (message) => {
    ...
    if (
      !content.startsWith(commandPrefix + "show") &&
      !content.startsWith(commandPrefix + "rank") &&
      !content.startsWith(commandPrefix + "top")
    )
      return;
    if (!message.guild) {
  ...
  if (subcommand.startsWith("server total")) {
    ...
    try {
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .addField(
          "bot",
          (await message.guild.members.fetch(targetUser)).user.bot
            ? "yes"
            : "no",
          true
        )
  ...
  if (subcommand.startsWith("server global")) {
    ...
    try {
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .setTitle(rank)
        .setDescription(
          "Rank of users from current server by their global karma"
        )
    ...
    try {
      let rank = await karmaRetriever.getGuildRankOfUser(
        targetUser,
        message.guild.id
      );
      let karma = await karmaRetriever.getTotalKarmaOfUserInGuild(
...
async function top(message, subcommand) {
  if (subcommand.startsWith("global")) {
    ...
    try {
      let topList = await karmaRetriever.getTopUsersOfGuildTotal(
        10,
        message.guild.id
      );
    ...
    try {
      let topList = await karmaRetriever.getTopUsersOfGuild(
        10,
        message.guild.id
      );
