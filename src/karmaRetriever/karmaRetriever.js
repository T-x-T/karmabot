...
export default {
  ...
  async getTotalKarmaHistory(hoursInPast){
    const history = await karmaReader.getTotalKarmaHistory(Date.now() - (hoursInPast * 60 * 60 * 1000));
  ...
  async getUserKarmaHistory(hoursInPast, userId){
    await throwIfUserDisabled(userId);
    const history = await karmaReader.getUserKarmaHistory(Date.now() - (hoursInPast * 60 * 60 * 1000), userId);
    const hisotryObjects = history.map(a => {
      return {
        userId: userId,
        timestamp: a.timestamp,
  ...
  async getGuildKarmaHistory(hoursInPast, guildId){
    await throwIfGuildDisabled(guildId);
    const history = await karmaReader.getGuildKarmaHistory(Date.now() - (hoursInPast * 60 * 60 * 1000), guildId);
    const hisotryObjects = history.map(a => {
      return {
        guildId: guildId,
        timestamp: a.timestamp,
  ...
  async getUserInGuildKarmaHistory(hoursInPast, userId, guildId){
    ...
    const history = await karmaReader.getUserInGuildKarmaHistory(Date.now() - (hoursInPast * 60 * 60 * 1000), userId, guildId);
    const hisotryObjects = history.map(a => {
      return {
        userId: userId,
        guildId: guildId,
