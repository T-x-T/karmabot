...
async function disableUser(message) {
  try {
    ...
    message.channel.send(
      `Disabled <@${message.author.id}>, their karma won't be counted and they won't be shown in any output anymore.\nTo enable again use ${commandPrefix}config enable`
    );
  } catch (e) {
    message.channel.send(
      `I am very sorry, this didn't work. Here is why: ${e.message}`
    );
  }
}
...
async function enableUser(message) {
  try {
    ...
    message.channel.send(
      `Enabled <@${message.author.id}>, their karma will be counted and they will be shown in output again.\nTo disable again use ${commandPrefix}config disable`
    );
  } catch (e) {
    message.channel.send(
      `I am very sorry, this didn't work. Here is why: ${e.message}`
...
async function disableGuildInUser(message) {
  if (!message.guild) {
    message.channel.send(
      "This command works only in servers at the moment. Please use this command in the server you want to disable."
    );
    return;
  }
  ...
  try {
    ...
    message.channel.send(
      `<@${message.author.id}> will not be receiving any karma in ${message.guild.name} and won't be shown in top lists of this guild\nTo enable again use ${commandPrefix}config enable server`
    );
  } catch (e) {
    message.channel.send(
      `I am very sorry, this didn't work. Here is why: ${e.message}`
...
async function enableGuildInUser(message) {
  if (!message.guild) {
    message.channel.send(
      "This command works only in servers at the moment. Please use this command in the server you want to enable."
    );
    return;
  }
  ...
  try {
    ...
    message.channel.send(
      `<@${message.author.id}> will receive karma in ${message.guild.name} and will be shown in top lists of this guild again\nTo disable again use ${commandPrefix}config disable server`
    );
  } catch (e) {
    message.channel.send(
      `I am very sorry, this didn't work. Here is why: ${e.message}`
...
async function disableGuild(message) {
  ...
  if (!(await authorIsAdmin(message))) {
    message.channel.send(
      "You must have the administrator permission to configure server"
    );
    return;
  }
  ...
  try {
    ...
    if (emojiId === "default") {
      try {
        ...
        message.channel.send(
          `The new upvote emoji of ${message.guild.name} is ${emoji}! React with this emoji to messages to upvote them.`
        );
      } catch (e) {
        console.log(e);
    ...
    if (isValidEmoji(emoji, message.guild.id)) {
      ...
      message.channel.send(
        "This isn't a valid emoji! You can only use custom emojis of this server."
      );
    }
  } catch (e) {
    message.channel.send(
  ...
  try {
    ...
    if (emojiId === "default") {
      try {
        ...
        message.channel.send(
          `The new downvote emoji of ${message.guild.name} is ${emoji}! React with this emoji to messages to downvote them.`
        );
      } catch (e) {
        console.log(e);
...
async function addDefaultUpvoteEmoji(guild) {
  return await guild.emojis.create(
    "./src/configurator/emoji_upvote.png",
    "upvote"
  );
}
...
async function addDefaultDownvoteEmoji(guild) {
  return await guild.emojis.create(
    "./src/configurator/emoji_downvote.png",
    "downvote"
  );
}
