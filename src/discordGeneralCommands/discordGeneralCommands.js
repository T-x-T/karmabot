...
export default async (
  ...
  const rest = new Discord.REST().setToken(botToken);

  let command_data = new Discord.SlashCommandBuilder()
    .setName("ping")
    .setDescription("Replies with Pong!");
  let command_execute = async (interaction) => {
  ...
  client.on(Discord.Events.MessageCreate, (message) => {
    ...
    if (
      !content.startsWith(commandPrefix + "help") &&
      !content.startsWith(commandPrefix + "info") &&
    ...
    if (content + " " === commandPrefix) {
      message.channel.send(
        `Hi, Im your friendly neighbourhood karmabot! To get help please type \`${commandPrefix}help\` or for general info about me \`${commandPrefix}info\`.`
      );
      return;
    }
  ...
  client.on("guildCreate", async (guild) => {
    ...
    if (!channel)
      await guild.channels.cache.array().forEach((_channel) => {
...
async function info(message) {
  const embed = new Discord.MessageEmbed()
    ...
    .addField(
      "send bug reports to",
      "incoming+t-x-t-karmabot-22359119-issue-@incoming.gitlab.com"
    )
    .setTimestamp()
