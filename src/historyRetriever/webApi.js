...
const handlers = [
  () => {
    router.register(
      ...
      async (req) => {
        const hoursInPast = req.query.hoursInPast
          ? req.query.hoursInPast
          : 24 * 7;
  ...
  () => {
    router.register(
      (req) =>
        req.path.startsWith("history/guild/") &&
        req.path.includes("/totalkarma"),
      async (req) => {
        const hoursInPast = req.query.hoursInPast
          ? req.query.hoursInPast
          : 24 * 7;
        const guildId = req.path.split("/")[2];
        return await historyRetriever.getGuildKarmaHistory(
          hoursInPast,
          guildId
        );
      }
  ...
  () => {
    router.register(
      ...
      async (req) => {
        ...
        const guildId = req.path.split("/")[2];
        return await historyRetriever.getUserInGuildKarmaHistory(
          hoursInPast,
          userId,
          guildId
        );
  ...
  () => {
    router.register(
      (req) =>
        req.path.startsWith("history/users/") &&
        req.path.includes("guildkarma"),
      async (req) => {
        const hoursInPast = req.query.hoursInPast
          ? req.query.hoursInPast
          : 24 * 7;
        const userId = req.path.split("/")[2];
        const resWithoutNames =
          await historyRetriever.getGuildKarmaOfAllGuildsOfUserHistory(
            hoursInPast,
            userId
          );
        return await Promise.all(
          resWithoutNames.map(async (entry) => {
            try {
              entry.guildName = await discordFetcher.getGuildNameById(
                entry.guildId
              );
            } catch (e) {
